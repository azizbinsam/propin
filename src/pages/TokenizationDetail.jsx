  ...
  if (!property) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate('/app/dashboard')} className="btn-primary mt-4">
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20 md:pb-8">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 badge-syariah mb-2">
          <CheckCircle size={14} /> Smart Contract Execution
        </div>
        <h1 className="text-2xl font-bold">Transaksi Tokenisasi</h1>
        <p className="text-neutral-500 text-sm">
          {property.title} · {tokenAmount} token · {formatRupiah(totalCost)}
        </p>
      </div>
      ...
      <div className="flex justify-center mb-6">
        {completed ? (
          <span className="bg-green-50 text-green-700 px-4 py-2 rounded-full text-sm font-semibold flex items-center gap-2">
            <CheckCircle size={18} /> Executed — Transaksi Selesai
          </span>
        ) : (
          <span className="bg-gold-50 text-gold-700 px-4 py-2 rounded-full text-sm font-semibold flex items-center gap-2 animate-pulse">
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-4">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-300 ${
                    isActive ? 'bg-green-600 text-white' : 'bg-neutral-200 text-neutral-400'
                  } ${isCurrent && !completed ? 'ring-4 ring-green-100 animate-pulse' : ''}`}
                >
      ...
      {completed && (
        <div className="mt-6 card bg-neutral-50">
          ...
          <div className="flex items-center gap-2 mt-1">
            <code className="text-sm font-mono bg-neutral-100 px-3 py-1 rounded-lg flex-1 truncate">
              {txHash}
            </code>
            <button
          ...
          <p className="text-xs text-neutral-400 mt-2">
            🔗 Transaksi tercatat di blockchain (simulasi prototype)
          </p>
        </div>
      )}
      ...
      {completed && (
        <div className="mt-6 flex flex-col sm:flex-row gap-4">
          ...
          >
            <ArrowRight size={18} /> Lihat Wallet
          </button>
          <button
            onClick={() => navigate(`/app/certificate/${property.id}`)}
